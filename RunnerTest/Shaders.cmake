...
function(compile_glsl_shaders GLSL_SHADER_SOURCES)
    foreach(SRC ${GLSL_SHADER_SOURCES})
        string(REGEX REPLACE "[.]glsl$" "_vert.spv" VS_OUT ${SRC})
        string(REGEX REPLACE "[.]glsl$" "_frag.spv" FS_OUT ${SRC})
        execute_process(COMMAND ${VK_BIN}/glslc -DVERTEX --target-spv=spv1.3 -fshader-stage=vert
                -o ${VS_OUT} ${SRC} RESULT_VARIABLE GLSLC_RESULT)
        if (GLSLC_RESULT AND NOT GLSLC_RESULT EQUAL 0)
            message(FATAL_ERROR " Failed to compile GLSL vertex shader. Error Code: " ${GLSLC_RESULT})
        endif()
        execute_process(COMMAND ${VK_BIN}/glslc -DFRAGMENT --target-spv=spv1.3 -fshader-stage=frag
                -o ${FS_OUT} ${SRC} RESULT_VARIABLE GLSLC_RESULT)
        if (GLSLC_RESULT AND NOT GLSLC_RESULT EQUAL 0)
            message(FATAL_ERROR " Failed to compile GLSL vertex shader. Error Code: " ${GLSLC_RESULT})
        endif()
        message(Shader Output: " ${VS_OUT} | ${FS_OUT}")
