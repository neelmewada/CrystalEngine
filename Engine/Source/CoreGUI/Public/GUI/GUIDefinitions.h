...
namespace CE::GUI
{
	...
	enum Key : int
	{
		...
		//   and prefer using the real keys (e.g. Key_LeftCtrl, Key_RightCtrl instead of KeyMod_Ctrl).
		// - In theory the value of keyboard modifiers should be roughly equivalent to a logical or of the equivalent left/right keys.
		//   In practice: it's complicated; mods are often provided from different sources. Keyboard layout, IME, sticky keys and
		//   backends tend to interfere and break that equivalence. The safer decision is to relay that ambiguity down to the end-user...
		KeyMod_None                   = 0,
		KeyMod_Ctrl                   = 1 << 12, // Ctrl
		KeyMod_Shift                  = 1 << 13, // Shift
		KeyMod_Alt                    = 1 << 14, // Option/Menu
		KeyMod_Super                  = 1 << 15, // Cmd/Super/Windows
		KeyMod_Shortcut               = 1 << 11, // Alias for Ctrl (non-macOS) _or_ Super (macOS).
		KeyMod_Mask_                  = 0xF800,  // 5-bits
	};
    ...
    enum class ButtonFlags
    {
		...
		NoSetKeyOwner = 1 << 20,  // don't set key/input owner on the initial click (note: mouse buttons are keys! often, the key in question will be Key_MouseLeft!)
		NoTestKeyOwner = 1 << 21,  // don't test key/input owner when polling the key (note: mouse buttons are keys! often, the key in question will be Key_MouseLeft!)
